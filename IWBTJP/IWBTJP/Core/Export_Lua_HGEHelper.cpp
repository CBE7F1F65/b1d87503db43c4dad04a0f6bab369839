...
int Export_Lua_HGEHelp::LuaFn_hgeFont_printfb(LuaState * ls)
{
	...
	SpriteItemManager::PIns()->FontPrintfb(_font, args[2].GetFloat(), args[3].GetFloat(), args[4].GetFloat(), args[5].GetFloat(), args[6].GetInteger(), args[7].GetString());
...
hgeEffectSystem * Export_Lua_HGEHelp::_Helper_New_hgeES(const char * filename, HTEXTURE tex /* = 0 */)
{
	hgeEffectSystem * _es = NULL;
	_es = new hgeEffectSystem(filename, tex, BResource::PIns()->tex);
	if (_es)
	{
		esList.push_back(_es);
	}
...
int Export_Lua_HGEHelp::LuaFn_hgeES_NewES(LuaState * ls)
{
	...
	if (argscount > 0)
	{
		...
		if (!args[1].IsNumber())
		{
			...
			_es = _Helper_New_hgeES(filename, _htexture);
		}
		else
		{
			_obj = args[1];
...
int Export_Lua_HGEHelp::LuaFn_hgeES_Render(LuaState * ls)
{
	...
	SpriteItemManager::PIns()->EffectSystemRender(_es, ptfar, colormask);
	*/
	SpriteItemManager::PIns()->EffectSystemRender(_es);
...
int Export_Lua_HGEHelp::LuaFn_hgeSprite_Render(LuaState * ls)
{
	...
	if (argscount > 2)
	{
		...
		SpriteItemManager::PIns()->RenderSpriteEx(_sprite, args[2].GetFloat(), args[3].GetFloat(), rot, hscale, vscale);
	}
	else
	{
		SpriteItemManager::PIns()->RenderSprite(_sprite, args[2].GetFloat(), args[3].GetFloat());
	}
...
int Export_Lua_HGEHelp::LuaFn_hgeSprite_GetTexture(LuaState * ls)
{
	...
	HTEXTURE _tex = _sprite->GetTexture();
	iret = _tex.texindex;
